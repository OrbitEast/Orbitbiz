...
(()=>{
  ...
  w.saveInvoice=async(event,state,node)=>{
    ...
    try{
      const result=await db.rpc("create_invoice_with_items",{p_business_id:state.businessId,p_customer_id:raw.customer_id,p_issue_date:raw.issue_date,p_due_date:raw.due_date||null,p_notes:raw.notes||null,p_items:items});
      if(result.error)throw result.error;
      const invoiceId=result.data;
      if(!invoiceId)throw Error("Invoice was created but its ID was not returned");
      node.remove();
      w.toast("Invoice created");
      // Use the exact RPC-returned invoice ID. Do not guess the latest invoice by customer.
      const detail=await w.invoiceDetailData(state,invoiceId);
      // Show the PDF design picker immediately after creation.
      if(typeof w.prepareInvoicePdf!=="function")throw Error("PDF engine is not available");
      await w.prepareInvoicePdf(state,detail);
      // Keep the new invoice connected to the normal workspace detail page after the PDF picker closes.
      state.page="invoices";
      state.invoiceId=invoiceId;
      state.invoiceCustomerId="";
      state._openNewInvoice=false;
      await state.renderPage();
    }catch(error){
      console.error("Invoice creation/PDF flow failed",error);
      w.toast(error.message||"Could not finish invoice creation");
      button.disabled=false;button.innerHTML=`Create invoice ${w.icon("arrow")}`;
    }
  };
})();
